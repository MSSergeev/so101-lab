#!/usr/bin/env python3
"""Evaluate trained ACT policy in Isaac Lab simulation.

This script runs a trained policy in simulation and collects statistics
...
Usage:
    ...
    python scripts/eval/eval_act_policy.py --checkpoint outputs/act_v1 --step 15000
    ...
    python scripts/eval/eval_act_policy.py --checkpoint outputs/act_v1 --save-episodes fail
...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Enable temporal ensembling with given coefficient (e.g., 0.01). Requires n_action_steps=1",
)
parser.add_argument(
    "--ensemble-interval",
    type=int,
...
def poll_command(pending_commands, use_preview):
    ...
    # Keyboard commands override preview
...
def run_episode(env, policy, device, episode_seed, recorder, ep_num, prev_successes,
    ...
    # (cameras may render stale scene from previous episode otherwise)
    ...
    while not done and step < args.max_steps:
        ...
        # --- Get action ---
        if args.temporal_ensemble_coeff is not None and args.ensemble_interval > 1 and step % args.ensemble_interval != 0:
            action = policy.pop_ensemble_actions()[0]
        else:
...
def main():
    ...
    # Load policy
    policy = ACTInference(checkpoint_path=str(checkpoint_path), device=device)
    ...
    # Enable temporal ensembling if specified
    if args.temporal_ensemble_coeff is not None:
        if args.n_action_steps is None:
            policy.config.n_action_steps = 1
            policy.policy.config.n_action_steps = 1
            print(f"  n_action_steps: auto-set to 1 for temporal ensembling")
        elif args.n_action_steps != 1:
            print(f"[WARNING] Temporal ensembling works best with n_action_steps=1, got {args.n_action_steps}")
        ...
        print(f"  ensemble_interval: {args.ensemble_interval}")
