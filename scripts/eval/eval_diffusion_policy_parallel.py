#!/usr/bin/env python3
"""Evaluate trained Diffusion Policy with parallel environments.

This script runs a trained diffusion policy across multiple parallel environments
...
Usage:
    ...
    python scripts/eval/eval_diffusion_policy_parallel.py --checkpoint outputs/diffusion_v1 --num-envs 4

    # More envs for faster evaluation
    python scripts/eval/eval_diffusion_policy_parallel.py --checkpoint outputs/diffusion_v1 --num-envs 8 --episodes 200

    # Override n_action_steps
    python scripts/eval/eval_diffusion_policy_parallel.py --checkpoint outputs/diffusion_v1 --n-action-steps 4
...
Notes:
    ...
    - Action queue mode only (no temporal ensembling for diffusion)
    - n_action_steps defaults to model config (same as training)
    - Diffusion inference is iterative (~100 denoising steps), so parallel eval
      is especially beneficial. Consider reducing --num-inference-steps for speed.
"""
...
parser = argparse.ArgumentParser(description="Evaluate Diffusion policy in parallel environments")
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Override number of diffusion denoising steps (default: from model config, try 10-20 for speed)",
)
...
class ParallelActionQueue:
    ...
    def __init__(self, num_envs: int, n_action_steps: int, policy: DiffusionInference):
...
def main():
    ...
    # Action queue mode (no temporal ensembling for diffusion)
    ...
    # Create action queue
    action_queue = ParallelActionQueue(num_envs, n_action_steps, policy)
