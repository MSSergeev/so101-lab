"""Sweep evaluation across multiple checkpoints.

Runs eval_act_policy_parallel.py sequentially for each checkpoint,
...
def build_eval_command(
    ...
    cmd = [
        sys.executable,
        str(Path(__file__).parent / "eval_act_policy_parallel.py"),
    ...
    if args.ensemble_interval != 1:
        cmd.extend(["--ensemble-interval", str(args.ensemble_interval)])
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep evaluation across multiple checkpoints",
    ...
    parser.add_argument("--temporal-ensemble-coeff", type=float, default=None,
                        help="Enable temporal ensembling with given coefficient")
    parser.add_argument("--ensemble-interval", type=int, default=1,
                        help="Run model every N steps when using temporal ensembling")
    ...
    # Seed handling: default = shared seed for fair comparison, "random" = independent per checkpoint
    if args.seed is None:
        ...
        args.seed = None  # each eval run will generate its own
    ...
    # --- Run evaluations ---
    # Pre-populate skipped checkpoints in results (in original order)
    results: list[dict] = [
    ...
    sweep_config = {
        ...
        "ensemble_interval": args.ensemble_interval,
