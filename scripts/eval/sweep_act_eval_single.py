"""Sweep evaluation across multiple checkpoints using single-env eval.

Runs eval_act_policy.py sequentially for each checkpoint,
collects results into a summary table and JSON.

Use this instead of sweep_act_eval.py when parallel eval has issues.
"""
...
def build_eval_command(
    ...
    cmd = [
        sys.executable,
        str(Path(__file__).parent / "eval_act_policy.py"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep evaluation across multiple checkpoints (single-env)",
    )
